...
- **Badges** (`BADGES` in `trophies.ts`): the old `everything-is-purple` plus milestones (`first-win`, `laps-100`, `laps-1000`, `gp-all-purple`, the streak badges `streak-7/14/30/50/100` generated from `STREAK_BADGE_DAYS`, `facts-50`, `superlicence`), evaluated by the pure `evaluateMilestones`. `superlicence` is the one badge not derivable from `GameState`: the licence lives in its own localStorage keys, and `gameLogic`/`trophies` cannot import `drivingSchoolLicence` (it would cycle via `drivingSchool` → `gameLogic`), so Hub passes `settleMilestones({ superlicence: true })` on mount — which also catches players who graduated before the badge existed. `applyMilestones` (gameLogic.ts) settles them from the saved state; `touchDailyStreak()` runs it on every counted session in any mode, and Game.tsx's `settleMilestones()` adds the mastery and Race Day ones. `announceRewards` (lib) sends one combined toast per session (text from the pure `rewardToast`), because the toaster shows one toast at a time (`TOAST_LIMIT = 1`) and separate toasts would replace each other. `BadgeTile` shows them; locked tiles keep their name, dimmed.
...
- **Daily streak** (`lib/dailyStreak.ts`, `GameState.dailyStreak`): consecutive local calendar days with a finished session; counted by `touchDailyStreak()` from the Game finish effect, a cleared flashcard stage, a completed Lane Racer race and a finished multiplayer race (not a crash). `DailyStreak.recentDays` keeps the last 7 counted days (nothing reads it since the card switched to streak-day dots). **Pit stops** (`DailyStreak.pitStops`, max 2): one is earned whenever the count reaches a multiple of 7 (not at the cap); when a session counts after missed days they are spent to cover all of them, or none (then the streak resets and keeps them). A covered day adds nothing to the count. `streakGap` is the single source for counting (`advanceDailyStreak`), `streakStatus`, `liveCount` and the card, so they never disagree; covered days are not stored. A gap of 0 or less (same day, a clock set back, a flight west) changes nothing. **Levels** (`streakLevel`): orange 1–6, bronze 7, silver 14, gold 30, purple 100, coloured by `STREAK_LEVEL_COLORS` (RewardStrip). `DailyStreakCard` tops `/trophies`: the live count in its level colour, pit stops held, seven dots numbered by streak day (`streakDots`: day 1 on the left, a new row every 7 days in step with pit stops, so "6 days to bronze" is six empty dots; today's dot is a dashed ring until it counts; a covered day adds no dot) and one line (`nextStreakGoal`, e.g. "14 days to gold and the 30-day badge"); at-risk amber only on the line and today's ring. The Hub shows no streak. Distinct from `GameState.streak`, the per-answer streak inside a race.
...
- Licence = all 10 stages + Reaction Test best under `REACTION_LICENCE_MS` (400 ms) + one Lane Racer win. It **gates nothing** — the Grand Prix is open to everyone — it earns the `superlicence` badge and shows the one-time `SuperlicenceSplash`. The school used to be a wall; the slope was too steep for a kid to reach the headline mode, so the gate was retired and the licence kept as an achievement. `grandPrixDevBypass` survives but now only skips the weekend's phase order (practice → qualifying → race) on the dev server.
