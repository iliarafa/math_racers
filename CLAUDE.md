...
- **Daily streak** (`lib/dailyStreak.ts`, `GameState.dailyStreak`): consecutive local calendar days with a finished session; counted by `touchDailyStreak()` from the Game finish effect, a cleared flashcard stage, a completed Lane Racer race and a finished multiplayer race (not a crash). `DailyStreak.recentDays` keeps the last 7 counted days. **Pit stops** (`DailyStreak.pitStops`, max 2): one is earned whenever the count reaches a multiple of 7 (not at the cap); when a session counts after missed days they are spent to cover all of them, or none (then the streak resets and keeps them). A covered day adds nothing to the count. `streakGap` is the single source for counting (`advanceDailyStreak`), `streakStatus`, `liveCount` and the card, so they never disagree; covered days are derived (gaps inside the current run), not stored. A gap of 0 or less (same day, a clock set back, a flight west) changes nothing. **Levels** (`streakLevel`): orange 1–6, bronze 7, silver 14, gold 30, purple 100, coloured by `STREAK_LEVEL_COLORS` (RewardStrip). `DailyStreakCard` tops `/trophies`: the live count in its level colour, pit stops held, the week as dots (P rings for covered or pending days) and one line (`nextStreakGoal`, e.g. "14 days to gold and the 30-day badge"); at-risk amber only on the line and rings. The Hub shows no streak. Distinct from `GameState.streak`, the per-answer streak inside a race.
