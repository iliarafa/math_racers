...
7. **Streak on `/trophies`, no emojis** (2026-09-19): the daily streak left the Hub for a card at the top of `/trophies` (count, best, the last 7 days as dots from `DailyStreak.recentDays`). The user set a standing rule of no emojis, so the reward rows, the personal-best banners and the leaderboard medals are now text and colour only. Streaks then gained badges at 14, 50 and 100 days, colour levels (bronze 7, silver 14, gold 30, purple 100) and pit stops (one per 7 days, hold 2, each covers a missed day); rewards now arrive in one combined toast.
