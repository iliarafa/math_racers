...
**Scope:** Gate the full lane slide behind a near-token distance band; early lane changes use soft flow

---

## Goal

Keep the continuous lane motion from the lane-flow polish, but reserve the long slide + full lean for **last-minute** steers when the answer row is close. Early lane changes should still flow (continuous X + light lean), not feel like the dramatic dodge every time.
...
| Early feel | Soft flow (B) — ~200ms, half lean |
...
| Mode | When | Duration | Yaw peak | Roll peak |
|------|------|----------|----------|-----------|
| Early (soft flow) | No tokens, or token `z` below near band | 200ms | 5° | 2.5° |
| Late (slide) | Tokens present and `z >= LATE_SLIDE_Z` | 300ms | 10° | 5° |

Shared for both modes:
- Continuous `carX` via `laneXVisual(carLaneVisual)`
- Ease-out cubic on `carLaneVisual`
...
1. Far from tiles or empty track: soft ~200ms flow with light lean — not the full slide.
...
- `/lane-racer` → 3D → Start
- Change lanes while tokens are far / missing → soft flow
